...
export function createGuestUser(guestProfile) {
  return postJson('api/users/guest', guestProfile)
}

export function getUserByDeviceId(deviceId) {
  return getJson(`api/users/guest/${deviceId}`)
}
...
export function getRequests(userId) {
  return getJson(`api/users/${userId}/requests`)
}
